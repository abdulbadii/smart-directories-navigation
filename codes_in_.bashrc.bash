...
[[ ${!#} = -0 ]] &&{
 ((HID=1-HID))
 if((HID)) ;then { echo -ne "DIRECTORY STACK LIST IS HIDDEN NOW\r";sleep 1.5;echo -ne "\e[K";}>&3;_DIRS=
 else _DIRS=$_DRS ;fi
 (($#==1)) && return 0
 set -- ${@:1:(($#-1))}
}
...
-[1-9]*([0-9])-) i=${1//-}; while popd +$i ;do :;done;;
--[1-9]*([0-9])) i=${1//-}; while ((i--));do popd -n;done;;
-[1-9]*([0-9])-[1-9]*([0-9]))
	m=${1%-*}; m=${m#-;};	n=${1##*-}
...
?*) F=1
  if [[ $(type -t -- $1) && $2 ]] ;then {
	[[ -d $1 ]] &&{
	echo "'$1' is a name of system executable and directory"
	read -N1 -p "Do you meant as executable or directory name (which must be with '/' at the end) ? (x or ELSE KEY) " o
	[[ $o = [xX] ]] ||{ pushd "$1";F=;}
	}
	((F)) &&{	x=$1;shift;	args=();DNO=
	for m;{
	...
	if [[ ${BASH_REMATCH[1]} ]] ;then pushd $1 ||echo $1/ is not a directory>&3
		...
		m=;[[ -d $1 ]]&&{
		m="Directory $1/ exists, to mean it instead, append '/' on CLI:  $1/\n"
